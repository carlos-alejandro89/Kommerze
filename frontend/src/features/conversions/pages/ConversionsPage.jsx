...
export function ConversionsPage() {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-[1480px] flex-col gap-4 p-5 lg:p-6" onClick={() => menu && setMenu('')}>
    <div className="flex justify-end"><button onClick={() => navigate('/conversions/new')} className="flex h-9 items-center gap-1.5 rounded-xl bg-primary px-3.5 text-[11px] font-semibold text-primary-foreground"><Plus className="size-3.5" /> Nueva conversión</button></div>
    <section className="flex min-h-[430px] flex-1 flex-col overflow-hidden rounded-2xl border border-white/70 bg-white/70 shadow-[0_18px_45px_-35px_rgba(20,54,110,.5)] dark:border-white/10 dark:bg-white/[.04]">
      <div className="flex flex-col justify-between gap-3 border-b border-border/70 px-5 py-4 md:flex-row md:items-center"><div><h2 className="text-sm font-semibold">Historial de conversiones</h2><p className="mt-1 text-[11px] text-muted-foreground">{advanced ? 'Resultados de la búsqueda avanzada.' : 'Conversiones realizadas durante la jornada actual.'}</p></div><div className="flex items-center gap-2"><div className="relative min-w-0 flex-1 md:w-[360px]"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar folio, producto o código…" className="h-10 w-full rounded-xl border border-border/70 bg-background/75 pl-10 pr-4 text-sm outline-none focus:border-primary/45 focus:ring-2 focus:ring-primary/10" /></div><button onClick={openFilters} className={cn('flex h-10 items-center gap-2 rounded-xl border px-3.5 text-xs font-semibold transition', activeCount ? 'border-primary/30 bg-primary/10 text-primary' : 'border-border/70 bg-background/75 text-muted-foreground hover:text-primary')}><SlidersHorizontal className="size-4" /><span className="hidden xl:inline">Búsqueda avanzada</span>{activeCount > 0 && <span className="flex size-5 items-center justify-center rounded-full bg-primary text-[10px] text-white">{activeCount}</span>}</button><button onClick={load} className="flex size-10 items-center justify-center rounded-xl border border-border/70 bg-background/75 text-muted-foreground hover:text-primary"><RefreshCw className={cn('size-4', loading && 'animate-spin')} /></button></div></div>
