...
export function ConversionsPage() {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-[1480px] flex-col gap-4 p-5 lg:p-6" onClick={() => menu && setMenu('')}>
    ...
    <nav className="flex items-center gap-2 text-[11px] font-medium text-muted-foreground"><button onClick={() => navigate('/home')}>Home</button><span>/</span><span className="text-foreground">Conversiones</span></nav>
    <header className="contents [&>div:first-child]:hidden">
      <div className="flex items-center gap-3"><div className="flex size-9 items-center justify-center rounded-xl bg-cyan-500/10 text-cyan-600"><Repeat2 className="size-4.5" /></div><div><h1 className="text-base font-bold">Conversiones</h1><p className="text-[11px] text-muted-foreground">Transforma existencias entre presentaciones mediante equivalencias configuradas.</p></div></div>
      <div className="flex gap-2"><button onClick={() => navigate('/home')} className="flex h-8 items-center gap-1.5 rounded-lg border border-border/70 px-3 text-[11px] font-semibold"><ArrowLeft className="size-3.5" /> Volver</button><button onClick={() => navigate('/conversions/new')} className="flex h-8 items-center gap-1.5 rounded-lg bg-primary px-3 text-[11px] font-semibold text-primary-foreground"><Plus className="size-3.5" /> Nueva conversión</button></div>
    </header>
    </div>
