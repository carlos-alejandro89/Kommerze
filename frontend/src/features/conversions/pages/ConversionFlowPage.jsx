...
export function ConversionFlowPage() {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-[1380px] flex-col gap-5 p-5 lg:p-6">
    ...
    <section className="flex min-h-[430px] flex-1 flex-col overflow-hidden rounded-2xl border border-white/70 bg-white/70 shadow-[0_18px_45px_-35px_rgba(20,54,110,.5)] dark:border-white/10 dark:bg-white/[.04]">
      ...
      <div className="flex min-h-11 items-center justify-between border-b border-border/55 bg-muted/[.16] px-5 py-2"><p className="text-[10px] text-muted-foreground">Mostrando <span className="font-semibold text-foreground">{visibleProducts.length}</span> conversión{visibleProducts.length === 1 ? '' : 'es'}</p><label className="flex cursor-pointer select-none items-center gap-2 text-[10px] font-medium text-muted-foreground transition hover:text-foreground"><span>Solo con existencia</span><span className={`relative h-4 w-7 rounded-full transition-colors ${onlyWithStock ? 'bg-primary' : 'bg-muted-foreground/25'}`}><input type="checkbox" checked={onlyWithStock} onChange={event => setOnlyWithStock(event.target.checked)} className="sr-only" /><span className={`absolute top-0.5 size-3 rounded-full bg-white shadow-sm transition-transform ${onlyWithStock ? 'translate-x-3.5' : 'translate-x-0.5'}`} /></span></label></div>
