...
import { ServiceConsultarProductosConvertibles, ServiceEjecutarConversion } from '../../../../wailsjs/go/main/App';
...
export function ConversionFlowPage() {
  ...
  const load = useCallback(async term => {
    ...
    try { setProducts(await ServiceConsultarProductosConvertibles((term || '').trim()) || []); }
  ...
  const invalid = !selected || numeric(quantity) <= 0 || numeric(quantity) > numeric(selected.existenciaOrigen);
  const open = item => { setSelected(item); setQuantity('1'); };
  const confirm = async () => {
    if (invalid) return;
    setSaving(true);
    try {
      const result = await ServiceEjecutarConversion({ reglaGuid: selected.reglaGuid, cantidad: numeric(quantity) });
      toast.success(`Conversión realizada: ${formatNumber(result?.cantidadDestino)} ${selected.unidadDestino || ''}`);
      setSelected(null); await load(search);
