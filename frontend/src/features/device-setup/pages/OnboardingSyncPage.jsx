...
import { CheckCircle2, CloudDownload, Loader2, MinusCircle, RefreshCw, XCircle } from 'lucide-react';
import {
...
export function OnboardingSyncPage() {
  ...
  const [items, setItems] = useState(() => CATALOGS.map(([name, , optional = false]) => ({ name, optional, status: 'pending' })));
  const [running, setRunning] = useState(false);
  const [error, setError] = useState('');

  const run = useCallback(async () => {
    setRunning(true); setError('');
    setItems(CATALOGS.map(([name, , optional = false]) => ({ name, optional, status: 'pending' })));
    for (let index = 0; index < CATALOGS.length; index += 1) {
      const [name, sync, optional = false] = CATALOGS[index];
      ...
      try {
        ...
        if (optional) {
          setItems((current) => current.map((item, i) => i === index ? { ...item, status: 'skipped', message: String(reason) } : item));
          continue;
        }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-6 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950">
      <motion.main initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl rounded-3xl border border-white/60 bg-background/95 p-8 shadow-2xl backdrop-blur-xl">
        ...
        <div className="grid max-h-[430px] grid-cols-1 gap-2 overflow-y-auto pr-2 sm:grid-cols-2">
          {items.map((item) => <div key={item.name} className="flex items-center gap-3 rounded-xl border border-border/70 bg-bg-subtle/60 px-3.5 py-3 text-sm" title={item.message || ''}><StatusIcon status={item.status} /><span className={item.status === 'pending' ? 'text-muted-foreground' : 'text-foreground'}>{item.name}</span>{item.optional && <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-[9px] font-semibold text-muted-foreground">Opcional</span>}</div>)}
        </div>
