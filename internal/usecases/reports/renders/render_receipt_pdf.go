...
func RenderReceiptPDF(r models.Receipt) ([]byte, error) {
	...
	if r.MostrarLogo && strings.TrimSpace(r.Logo) != "" {
		if logo, ok := decodeReceiptLogo(r.Logo); ok {
			...
			headerExtraHeight += logoHeight + 1.3
		}
	}
	...
	if r.MostrarLogo {
		if logo, ok := decodeReceiptLogo(r.Logo); ok {
			...
			if pdf.Error() == nil && logoWidth > 0 && logoHeight > 0 {
				...
				// Aproximadamente cinco píxeles visuales entre imagen y encabezado.
				pdf.Ln(logoHeight + 1.3)
			}
		}
	}
	...
	// Dos puntos por encima del resto de importes del bloque de totales.
	...
	// Reserva visual equivalente a unos tres píxeles al final del ticket.
	pdf.Ln(.8)
